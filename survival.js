...
function initializeSurvival(){if(resourceNodes.length)return;for(const t of trees){t.hp=6;t.maxHp=6;}const rand=mulberry32(994211);for(let i=0;i<230;i++){const kind=i<95?'stone':i<190?'fiber':'wood';let pos=null;for(let a=0;a<50;a++){const x=mapBounds.minX+2+rand()*(mapBounds.maxX-mapBounds.minX-4),z=mapBounds.minZ+2+rand()*(mapBounds.maxZ-mapBounds.minZ-4),y=originalHeightAt(x,z);if(y!==null&&!obstacles.some(o=>Math.hypot(o.pos.x-x,o.pos.z-z)<o.radius+.65)){pos=new THREE.Vector3(x,y,z);break;}}if(!pos)continue;const g=new THREE.Group();if(kind==='stone'){for(let j=0;j<3;j++){const m=new THREE.Mesh(new THREE.DodecahedronGeometry(.25+rand()*.16,0),new THREE.MeshStandardMaterial({color:0x8297a3,roughness:.9}));m.scale.y=.65;m.position.set((rand()-.5)*.5,.16,(rand()-.5)*.5);m.castShadow=true;g.add(m);}}
